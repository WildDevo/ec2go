...
// keyToBytes converts a bubbletea v2 key event into the byte sequence a
// terminal would send to the child process. Adapted from taigrr/bubbleterm
// (ISC-licensed, Copyright (C) 2025 Tai Groot); it uses the structured key
// fields (Code, Mod, Text) rather than string matching so every modifier
// combination is handled programmatically.
...
func keyToBytes(msg tea.KeyPressMsg) string {
	k := msg.Key()
	...
	// Printable text (regular characters, shift+letter, symbols).
	if k.Text != "" {
		if mod&tea.ModAlt != 0 {
			return "\x1b" + k.Text
		}
...
// modParam returns the xterm modifier parameter: 1 + shift + alt*2 + ctrl*4.
...
// csiTilde returns CSI tilde sequences for insert/delete/pgup/pgdown/F5-F12.
